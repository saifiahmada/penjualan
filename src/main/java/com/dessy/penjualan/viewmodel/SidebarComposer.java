package com.dessy.penjualan.viewmodel;

import java.util.List;


import org.zkoss.zk.ui.Component;
import org.zkoss.zk.ui.event.Event;
import org.zkoss.zk.ui.event.EventListener;
import org.zkoss.zk.ui.event.Events;
import org.zkoss.zk.ui.select.SelectorComposer;
import org.zkoss.zk.ui.select.annotation.Listen;
import org.zkoss.zk.ui.select.annotation.Wire;
import org.zkoss.zk.ui.util.Clients;
import org.zkoss.zkmax.zul.Navbar;
import org.zkoss.zkmax.zul.Navitem;
import org.zkoss.zkplus.spring.SpringUtil;
import org.zkoss.zul.A;
import org.zkoss.zul.Div;
import org.zkoss.zul.Hlayout;
import org.zkoss.zul.Include;
import org.zkoss.zul.Vlayout;

import com.dessy.penjualan.bean.MstMenu;
import com.dessy.penjualan.util.MstService;

public class SidebarComposer extends BasePage {
	
	@Wire
	Hlayout main;
	@Wire
	Div sidebar;
	@Wire
	Navbar navbar;
	@Wire
	Navitem calitem;
	@Wire
	A toggler;
	
	public void doAfterCompose(Component comp) throws Exception {
		super.doAfterCompose(comp);
		
		List<MstMenu> listMenu = getMstService().getMstMenuService().findOrderer();
		
		for (MstMenu menu : listMenu) { 
			Navitem item = new Navitem();
			
			item.setLabel(menu.getVtitle()); 
			item.setIconSclass(menu.getVimage());
			final String vlocation = menu.getVlocation(); 
			item.addEventListener(Events.ON_CLICK, new EventListener<Event>() {
				
				public void onEvent(Event arg0) throws Exception {
					Hlayout hlay = (Hlayout) sidebar.getParent().getParent();
					Vlayout vlay = (Vlayout) hlay.getFellowIfAny("vlay");
					Include include = (Include) vlay.getFellowIfAny("mainmenu");
					include.setSrc(vlocation);
				}
				
			});
			item.setParent(navbar); 
		}
		
	}
	
	// Toggle sidebar to collapse or expand
	@Listen("onClick = #toggler")
	public void toggleSidebarCollapsed() {
		if (navbar.isCollapsed()) {
			sidebar.setSclass("sidebar");
			navbar.setCollapsed(false);
			
			toggler.setIconSclass("z-icon-angle-double-left");
		} else {
			sidebar.setSclass("sidebar sidebar-min");
			navbar.setCollapsed(true);
			
			toggler.setIconSclass("z-icon-angle-double-right");
		}
		// Force the hlayout contains sidebar to recalculate its size
		Clients.resize(sidebar.getRoot().query("#main"));
	}
}